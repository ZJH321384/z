...
// 生成完整试卷题目
function generateFullPaperQuestions(subject, count) {
  const questions = [];
  const types = ['choice', 'fillblank', 'truefalse', 'subjective'];
...
async function savePaperToDB(paper) {
  return new Promise((resolve, reject) => {
    ...
    // 生成完整的题目
    const questions = generateFullPaperQuestions(paper.subject, paper.questionCount);
    ...
    db.run(
      ...
      function(err) {
        ...
        stmt.finalize();
        resolve({ paperId, questionCount: questions.length });
      }
    );
  });
}
...
async function main() {
  ...
  for (const paper of papersData) {
    try {
      ...
      const result = await savePaperToDB(paper);
      console.log(`✅ 成功保存: ${paper.subject} ${paper.year}年 ${paper.semester} - ${result.questionCount}道题\n`);
      successCount++;
  ...
  const subjects = {};
  const years = {};
  const types = { choice: 0, fillblank: 0, truefalse: 0, subjective: 0 };
