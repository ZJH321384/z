...
// 保存考试到数据库
async function saveExamToDB(paper) {
  return new Promise((resolve, reject) => {
    ...
    db.run(
      ...
      function(err) {
        ...
        stmt.finalize();
        resolve({ examId, questionCount: questions.length, totalScore });
      }
    );
  });
}
...
// 主函数
async function main() {
  console.log('开始导入大湾区高一联考试卷到考试系统...\n');
  ...
  for (const paper of papersData) {
    try {
      console.log(`正在导入: ${paper.title}`);
      const result = await saveExamToDB(paper);
      console.log(`✅ 成功导入: ${paper.subject} - ${result.questionCount}道题，总分${result.totalScore}分\n`);
      successCount++;
      totalQuestions += result.questionCount;
    } catch (error) {
      console.error(`❌ 导入失败: ${paper.title}`, error.message);
      failCount++;
    }
  }
